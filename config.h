...
/*
 * TPROXY.  A seperated daemon to do ssl offloading.  The idea is that you mark a P line with the P flag, so ircd knows it is an ssl port
 * ircd will then proxy all communication trough the seperated daemon, which will in it's turn put unencrypted stuff to the ircd again.
 */
#define USE_TPROXY
 ...
 */
#define MAXCHANNELSPERUSER  35	/* Recommended value: 35 */
...
** define this.
*/
#define DISABLE_NICK0_REGISTRATION
...
/*
** Vantage specific:
** GLINE_EMAIL: the email adres accepting complaints about emails
** GLINE_URL: the url the visitor can visit for gline details, include %s to be replaced with id
*/

#define GLINE_EMAIL	"abuse@krey.net"
#define GLINE_URL	"http://www.krey.net/hc/gline.html?id=%s"

/* Some webchats might need this to spoof hostnames
 * btw: CHANGE THE PASSWORD IF YOU DEFINE THIS!
 */
 
#define SETHOST

#ifdef SETHOST
#define SETHOST_PASS "FSDf4925723hrf"
/* upto 3 different IP's can be specified who can use SETHOST, so if you need
   more, define SETHOST_IP2 and/or SETHOST_IP3 */
#define SETHOST_IP1   "127.0.0.1" /* only this ip is allowed to spoof */
#endif
...
*/
#define LOCALNICKLEN 12

/*
** /map and /links wil display this text to make sure you can properly advertise the servers that are available
*/
#define SERVERLIST_MESSAGE "The official list of servers can be found at http://www.krey.net/"


/*
** If you wish, by defining this, people can only join channels that are already created.
** you must link to another server that creates the channels for you, otherwise no channels can be joined
*/

#undef ONLY_EXISTING_CHANNELS
...
#ifndef HAVE_OPENSSL
#  error No openssl - undef USE_TPROXY in config.h
#endif
...
/* if nickdelay is not accepteable,it can be disabled here */
#undef DISABLE_NICK_DELAY
